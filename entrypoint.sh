...
if [ "$HDFS_MASTER" != "" ]; then
	...
	mv /opt/spark-2.0.2-bin-hadoop2.7/conf/core-site.xml.datalake /opt/hadoop/etc/hadoop/core-site.xml
fi
...
if [ "$SPARK_CONTAINER_DIR" != "" ]; then
    ...
    wget https://github.com/bigstepinc/datalake-client-libraries/releases/download/untagged-81713202b662c6ab2bf3/datalake-client-libraries-1.4-SNAPSHOT.jar -P /opt/spark-2.0.2-bin-hadoop2.7/jars/
    ...
    wget http://central.maven.org/maven2/org/apache/spark/spark-streaming-kafka-0-8-assembly_2.11/2.0.1/spark-streaming-kafka-0-8-assembly_2.11-2.0.1.jar -P /opt/spark-2.0.2-bin-hadoop2.7/jars/
    cp /opt/spark-2.0.2-bin-hadoop2.7/jars/spark-streaming-kafka-0-8-assembly_2.11-2.0.1.jar $HADOOP_HOME/share/hadoop/common/
...
sed "s/HOSTNAME_MASTER/$SPARK_MASTER_HOSTNAME/" /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf.template >> /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf.tmp && \
mv /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf.tmp /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf

sed "s/SPARK_UI_PORT/$SPARK_UI_PORT/" /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf >> /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf.tmp && \
mv /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf.tmp /opt/spark-2.0.2-bin-hadoop2.7/conf/spark-defaults.conf
...
CLASSPATH=/opt/spark-2.0.2-bin-hadoop2.7/jars/
